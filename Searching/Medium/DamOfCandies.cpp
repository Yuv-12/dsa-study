...
it has N books, each of whose height is represented by the array height[] and
has unit width.
...
The task is to find out the area between two books that can hold the maximum
...
int maxCandy(int height[], int n)
{
    ...
    while (low <= high)
    {
        current_area = (high - low - 1) * (min(height[low], height[high]));
